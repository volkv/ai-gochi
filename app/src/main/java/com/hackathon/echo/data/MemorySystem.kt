...
class MemorySystem(private val context: Context) {
        ...
        return when {
            lastEmotion != null -> {
                ...
                when (emotionType) {
                    ...
                    EmotionType.NEUTRAL -> "Доброе утро! Надеюсь, у тебя все спокойно, как и вчера 😊"
    ...
    private fun getPatternBasedResponse(emotion: EmotionType, recentEmotions: List<StoredEmotion>): String {
        ...
        return when (emotion) {
            ...
            EmotionType.NEUTRAL -> when {
                consecutiveCount >= 3 -> "Ты в спокойном, уравновешенном состоянии... Хорошо! 😌"
                else -> "Все по-прежнему спокойно... Иногда это именно то, что нужно 😊"
            }
    ...
    private fun getDetailedInputResponse(emotion: EmotionType, userInput: String): String {
        return when (emotion) {
            ...
            EmotionType.NEUTRAL -> "Твое подробное сообщение в спокойном тоне очень приятно читать. Спасибо! 😊"
    ...
    private fun getFirstTimeEmotionResponse(emotion: EmotionType): String {
        return when (emotion) {
            ...
            EmotionType.NEUTRAL -> "Приятно видеть тебя в спокойном, уравновешенном состоянии! 😊"
            else -> ResponseBank.getRandomResponse(emotion)
        }
    }
    ...
    private fun isSignificantEmotionChange(from: EmotionType, to: EmotionType): Boolean {
        val emotionPairs = listOf(
            ...
            EmotionType.THOUGHTFUL to EmotionType.NEUTRAL,
            EmotionType.NEUTRAL to EmotionType.THOUGHTFUL
        )
    ...
    private fun getTransitionQuestion(from: EmotionType, to: EmotionType): String {
        return when (from to to) {
            ...
            EmotionType.THOUGHTFUL to EmotionType.NEUTRAL -> 
                "От глубоких размышлений к спокойствию... Пришел к каким-то выводам?"
            EmotionType.NEUTRAL to EmotionType.THOUGHTFUL -> 
    ...
    private fun getContinuityQuestion(emotion: EmotionType): String {
        return when (emotion) {
            ...
            EmotionType.NEUTRAL -> "Ты остаешься в спокойном состоянии... Все хорошо?"
